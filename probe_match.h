...
public:
	...
	bool do_alignment;
	int match_score, sub_penalty, ins_penalty, del_penalty;
...
inline bool probe_match::initialize(){
	...
	rebuild:
		...
		for (i = 0; i < num_probes; i++) {
			...
			if (!in) panic("error");
			if (num_mismatch > 0 && hash_1bp_mismatch) {
				uint jj = rand()%probe_len;
				uint k = rand()%4;
				temp = change_probe(temp, jj, k);
				vector<uint> indexes;
				bool in = false;
				if (hash_detect_probe(temp, indexes) != (uint)(-1)) {
					for (j = 0; j < (int)indexes.size(); j++)
...
//test speed
	...
	clock_t clock2 = clock();
	if (verbose) cout << "estimated search speed is " << i/difftime_clock(clock2, clock1) << " bps/sec.\n";
...
inline bool probe_match::initialize(vector<string> &probe_sequences, vector<string> &probe_tags){
	...
	probe_len = min_len;
	if (cut_start > 0) {
		if (cut_end > 0) probe_len = min(cut_end - cut_start + 1, probe_len);
		else probe_len = probe_len - cut_start + 1;
	}
	if (min_match > 0) probe_len = min_match;
	...
	for (int i = 0; i < (int)probe_sequences.size(); i++) {
		...
		if (cut_start > 0) {
			if (cut_end > 0) probe = probe.substr(cut_start - 1, cut_end - cut_start + 1);
			else probe = probe.substr(cut_start - 1);
		}
...
inline bool probe_match::search_file(const string &fa_file_name,  search_result_handler handler){
	...
	if (!file_exists(fa_file_name))  panic("transcript file doesn't exist.\n");
	if (!is_fasta_file(fa_file_name, false)) panic("transcript file is not a valid FASTA file.\n");
	...
	fasta myfasta;
	if (!myfasta.get_first_sequence(fa_file_name, transcript_id, line)) {
		panic("read fasta file failed.\n");
	}
	...
	while (true) {
			...
			if (search_key(key, indexes)) {
				for (j = 0; j < (int)indexes.size(); j++) {
					...
					if (probes[indexes[j]].rematch || current_pos > (int)line.length()) {
						...
						temp_result.num_mismatch = mismatch(temp_result.trans_seq, probe_seq, min_match, num_mismatch, num_insdel, temp_result.trans_key_len);
						if (temp_result.num_mismatch > num_mismatch) continue;
						if (do_alignment) {
							int temp_len;
							temp_result.score = local_alignment(temp_result.trans_seq, probe_seq, temp_result.trans_key_len, temp_len, 0, (min_match>0)?min_match:(int)probe_seq.length(), num_insdel > 0, false, match_score, sub_penalty, ins_penalty, del_penalty);
						}
						...
						if (do_alignment) {
							string probe1 = tostring(key), probe2 = tostring(probes[indexes[j]].key);
							int temp_len;
							temp_result.score = local_alignment(probe1, probe2, temp_result.trans_key_len, temp_len, 0, (min_match>0)?min_match:(int)probe2.length(), num_insdel > 0, false, match_score, sub_penalty, ins_penalty, del_penalty);
						}
					...
					temp_result.reverse_strand = reverse_strand;
					if (temp_result.reverse_strand) {
						temp_result.trans_coord = (int)line.length() - temp_result.trans_coord + (zero_indexed?0:2) - temp_result.trans_key_len;
						temp_result.trans_seq = get_reverse2(temp_result.trans_seq);
						temp_result.trans_key = get_reverse(temp_result.trans_key);
					}
		...
		if (search_both_strands && !reverse_strand) {
			reverse_strand = true;
			line = get_reverse2(line);
		} else {
			if (!myfasta.get_next_sequence(transcript_id, line)) {
				break;
			}
