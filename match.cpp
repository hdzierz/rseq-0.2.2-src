...
class ion_search_result_struct{
public:
	int trans_id;
	int trans_coord;
	string trans_seq;
...
vector<vector<ion_search_result_struct> > ion_top_matches;
...
void file_write_handler(vector<search_result_struct> &results) {
	...
	for (i = 0; i < (int)results.size(); i++) {
		...
		if (statistics_only) {
			...
			if (do_ion) {
				j = 0;
				for (j = 0; j < (int)ion_top_matches[results[i].probe_index].size(); j++) {
					if (results[i].score < ion_top_matches[results[i].probe_index][j].score) {
						break;
					}
				}
				if (j < num_top_matches) {
					ion_search_result_struct result;
					result.trans_id = results[i].trans_id;
					result.trans_coord = results[i].trans_coord;
					result.reverse_strand = results[i].reverse_strand;
					result.trans_seq = results[i].trans_seq;
					result.score = results[i].score;
					ion_top_matches[results[i].probe_index].insert(ion_top_matches[results[i].probe_index].begin() + j, result);
				}
				if ((int)ion_top_matches[results[i].probe_index].size() > num_top_matches) {
					ion_top_matches[results[i].probe_index].erase(ion_top_matches[results[i].probe_index].begin() + num_top_matches);
					__ASSERT((int)ion_top_matches[results[i].probe_index].size() == num_top_matches, "internal error: wrong num_top_matches.\n");
				}
			} else {
			...
			if (output_alignment){
				int temp1 = 0, temp2 = 0;
				local_alignment(trans_seq, probe_seq, temp1, temp2, (int)trans_seq.length(), (myprobe_match.min_match>0)?myprobe_match.min_match:(int)probe_seq.length(), myprobe_match.num_insdel > 0, true, myprobe_match.match_score, myprobe_match.sub_penalty, myprobe_match.ins_penalty, myprobe_match.del_penalty);
			}
...
void do_statistics(){
	for (int i = 0; i < num_probes; i++) {
		if (myprobe_match.probes[i].repeat >= 0) {
			...
			if (do_ion) {
				ion_top_matches[i] = ion_top_matches[mirror_probe];
			} else {
	...
	if (do_ion) {
		...
		for (i = 0; i < num_probes; i++) {
			for (j = 0; j < (int)ion_top_matches[i].size(); j++) {
				string seq1 = myprobe_match.probes[i].seq;
				string seq2 = ion_top_matches[i][j].trans_seq;
				int probe_len = (int)seq1.length();
				if (ion_top_matches[i][j].reverse_strand) seq2 = get_reverse2(seq2);
				int temp_len = 0;
				int score = local_alignment(seq2, seq1, temp_len, probe_len, (int)seq2.length(), (myprobe_match.min_match>0)?myprobe_match.min_match:(int)seq2.length(), myprobe_match.num_insdel > 0, true, myprobe_match.match_score, myprobe_match.sub_penalty, myprobe_match.ins_penalty, myprobe_match.del_penalty);
				int num_identity = 0;
				int num_mismatch = 0;
				int num_gap = 0;
				for (int k = 0; k < (int)seq1.length(); k++) {
					if (seq1[k] >= 'A' && seq1[k] <= 'Z') num_identity++;
					else if (seq1[k] == '_' && (k == 0 || seq1[k-1] != '_')) num_gap++;
					else if (seq2[k] != '_') num_mismatch++;
				}
				__ASSERT(score == ion_top_matches[i][j].score, "internal error, wrong score\n");
				file << myprobe_match.probes[i].id;
				file << "\t" << myprobe_match.transcripts[ion_top_matches[i][j].trans_id];
				file << "\t" << (double)num_identity/seq2.length()*100;
				file << "\t" << (int)seq2.length();
				file << "\t" << num_mismatch;
				file << "\t" << num_gap;
				file << "\t" << "0";
				file << "\t" << probe_len - 1;
				file << "\t" << ion_top_matches[i][j].trans_coord;
				file << "\t" << ion_top_matches[i][j].trans_coord + temp_len - 1;
				file << "\t" << "0";
				file << "\t" << -score;
				file << endl;
			}
...
int main(int argc, char* argv[]){
	...
	if (argc > 1 && argc < 5) {
		...
		printf("[/ion]                                         \toutput in ion style\n");
		...
		printf("[/min_match:len]                               \tmininum length of match\n");
		printf("[/score:match,sub,ins,del]                     \tscore for local alignment\n");
	...
	for (i = 5; i < argc; i++) {
		...
		if (command == "forward_strand") {
		...
		} else if (command == "min_match") {
			myprobe_match.min_match = str2int(parameter);
			...
			vector<string> tokens = string_tokenize(parameter, ",", false);
			if (tokens.size() != 2) panic("bad cut parameters.\n");
			if (!is_int(tokens[0])) panic("bad cut parameters.\n");
			myprobe_match.cut_start = str2int(tokens[0]);
			if (tokens[1] != "") {
				if (!is_int(tokens[1])) panic("bad cut parameters.\n");
				myprobe_match.cut_end = str2int(tokens[1]);
			}
			if (!(myprobe_match.cut_start >= 1 && (myprobe_match.cut_end == 0 || myprobe_match.cut_end > myprobe_match.cut_start))) panic("bad cut parameters.\n");
			...
			num_top_matches = 1;
			output_alignment = true;
			myprobe_match.do_alignment = true;
			output_probe_without_match = false;
			myprobe_match.min_match = 25;
			myprobe_match.match_score = -1;
			myprobe_match.sub_penalty = 10000;
			myprobe_match.ins_penalty = 2;
			myprobe_match.del_penalty = 2;
		} else if (command == "score") {
			vector<string> tokens = string_tokenize(parameter, ",", false);
			if (tokens.size() != 4) panic("bad penalty parameters.\n");
			if (!is_int(tokens[0]) || !is_int(tokens[1]) || !is_int(tokens[2]) || !is_int(tokens[3])) panic("bad penalty parameters.\n");
			myprobe_match.match_score = str2int(tokens[0]);
			myprobe_match.sub_penalty = str2int(tokens[1]);
			myprobe_match.ins_penalty = str2int(tokens[2]);
			myprobe_match.del_penalty = str2int(tokens[3]);
	...
	if (do_ion) {
		myprobe_match.num_insdel = myprobe_match.num_mismatch;
	}
	...
	if (statistics_only) {
		if (do_ion) {
			myprobe_match.extra_memory_per_probe = sizeof(vector<ion_search_result_struct>) + num_top_matches * sizeof(ion_search_result_struct); //ion_top_matches;
		} else {
	...
	num_probes = myprobe_match.num_probes;
	match_counts = NULL;

	if (statistics_only) {
		if (do_ion) {
			ion_top_matches.resize(num_probes);
		} else {
