...
inline string join_strings(const vector<string> &strings, const string separator = "") {
	string result = "";
	for (int i = 0; i < (int)strings.size(); i++) {
		result += strings[i];
		if (i < (int)strings.size() - 1) result += separator;
	}
	return result;
}
...
inline bool is_empty_str(const string s) {
	return (trim_space(s) == "");
}
