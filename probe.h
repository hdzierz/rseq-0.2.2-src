...
inline int nt2int(char ch) {
	ch = (char)toupper(ch);
	int j;
	for (j = 0; j < 4; j++) {
		if (ch == DNA_C[j]) return j;
	}
	return -1;
}
...
inline int local_alignment(string &input_probe1, string &input_probe2, int &len_1, int &len_2, int min_len1, int min_len2, int do_insdel, bool output_alignment, int match_score, int sub_penalty, int ins_penalty, int del_penalty) {
	string probe1 = toupper(input_probe1);
	string probe2 = toupper(input_probe2);
	...
	if (!do_insdel) {
		...
		vector<vector<int> > d;
		d.resize(len1+1);
		for (i = 0; i < (int)d.size(); i++) d[i].resize(len2+1);
		for (i = 0; i <= len1; i++) d[i][0] = i * del_penalty;
		for (j = 0; j <= len2; j++) d[0][j] = j * ins_penalty;

		for (i = 1; i <= len1; i++) {
			for (j = 1; j <= len2; j++) {
				int cost = (probe1[i-1] == probe2[j-1]) ? match_score : sub_penalty;
				d[i][j] = min(d[i-1][j]+del_penalty, d[i][j-1]+ins_penalty);
				d[i][j] = min(d[i][j], d[i-1][j-1]+cost);
			}
		}
		len_1 = len1;
		len_2 = len2;
		result = d[len_1][len_2];
		for (i = min_len1; i <= len1; i++) {
			for (j = min_len2; j <= len2; j++) {
				if (d[i][j] < result) {
					result = d[i][j];
					len_1 = i;
					len_2 = j;
		...
		if (!output_alignment) return result;
		while (i > 0 || j > 0) {
			if (i > 0 && j > 0 && d[i][j] == d[i-1][j-1] + sub_penalty) {
				i--;
				j--;
				seq1.insert(seq1.begin(), (char)tolower(probe1[i]));
				seq2.insert(seq2.begin(), (char)tolower(probe2[j]));
			} else if (i > 0 && d[i][j] == d[i-1][j] + del_penalty) {
				i--; 
				seq1.insert(seq1.begin(), (char)tolower(probe1[i]));
				seq2.insert(seq2.begin(), '_');
			} else if (j > 0 && d[i][j] == d[i][j-1] + ins_penalty) {
				j--;
				seq1.insert(seq1.begin(), '_');
				seq2.insert(seq2.begin(), (char)tolower(probe2[j]));
			} else if (i > 0 && j > 0 && d[i][j] == d[i-1][j-1] + match_score) {
				i--;
				j--;
				seq1.insert(seq1.begin(), probe1[i]);
				seq2.insert(seq2.begin(), probe2[j]);
			} else {
				cout << input_probe1 << endl;
				cout << input_probe2 << endl;
				cout << i << ", " << j << ", " << d[i][j] << endl;
				panic("internal error: local_alignment\n");
			}
		}
	}
	if (output_alignment) {
...
inline int _edit_dist3(const string &probe1, const string &probe2, int min_match, int num_mismatch, int num_insdel, int &len) {
	int i, j;
	vector<vector<int> > d;
	...
	for (i = 1; i <= probe1_len; i++) {
		...
		while (j <= i + num_insdel && j <= probe2_len) {
			...
			d[i][j] = min(d[i][j], d[i-1][j-1]+cost);
			if (((min_match == 0 && j == probe2_len) || (min_match > 0 && j == min_match) || (min_match > 0 && j == probe2_len && j < min_match)) && d[i][j] < min_d) {
				min_d = d[i][j];
				min_d_i = i;
			}
			if (i == j && d[i][j] > num_mismatch + num_insdel && (min_match == 0 || i <= min_match)) return (num_mismatch + 1);
			j++;
		}
	}
...
inline int mismatch(const string &seq1, const string &seq2, int min_match, int num_mismatch, int num_insdel, int &len) {
	if (num_insdel > 0) return _edit_dist3(seq1, seq2, min_match, num_mismatch, num_insdel, len);
	else {
		if (min_match > 0) return _mismatch2(seq1.substr(0, min_match), seq2.substr(0, min_match), num_mismatch);
